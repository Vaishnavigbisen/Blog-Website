...
export default function Profile(props) {
  ...
  const { profile, appErr, serverErr, follow, unfollow, userAuth } = user;
  //dispatch
  useEffect(() => {
  ...
  //Chek if the login user is the same as the user in the url
  const isLoginUser = userAuth?._id === id;

  return (
    <>
      {appErr || serverErr ? (
        <DisplayErrorMessage msg={(serverErr, appErr)} url="/users" />
      ) : (
